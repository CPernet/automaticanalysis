...
TORQUE=${ANTSPATH}waitForTorqueJobs.pl
...
Optional arguments:
     ...
     -c:  Control for parallel computation (default 1) -- 0 == run serially,  1 == SGE qsub,
          2 == use PEXEC (localhost),  3 == Apple XGrid, 4 == PBS qsub, 5 == Torque qsub
...
Optional arguments:
     ...
     -c:  Control for parallel computation (default 1) -- 0 == run serially,  1 == SGE qsub,
	  2 == use PEXEC (localhost), 3 == Apple XGrid, 4 == PBS Grid, 5 == Torque qsub
...
while getopts "c:d:g:i:j:h:m:n:o:p:s:r:t:x:z:" OPT
  ...
  case $OPT in
	  ...
	  DOQSUB=$OPTARG
	  if [[ ${#DOQSUB} -gt 2 ]] ; then
	      echo " DOQSUB must be an integer value (0=serial, 1=SGE qsub, 2=try pexec, 3=XGrid, 4=PBS qsub, 5=Torque qsub ) you passed  -c $DOQSUB "
	      exit 1
	  fi
	  ;;
      d) #dimensions
...
if [ "$RIGID" -eq 1 ] ;
    ...
    for IMG in $IMAGESETVARIABLE
      ...
      if [ $DOQSUB -eq 1 ] ; then
			...
			echo "cp -R /jobtmp/pbstmp.\$PBS_JOBID/* ${currentdir}" >> $qscript;
    	  	id=`qsub -N antsrigid -v ITK_GLOBAL_DEFAULT_NUMBER_OF_THREADS=1,LD_LIBRARY_PATH=$LD_LIBRARY_PATH,ANTSPATH=$ANTSPATH $QSUBOPTS -l mem=4gb -l walltime=4:00:00 $qscript | awk '{print $1}'`
	      	jobIDs="$jobIDs $id"
	  	    sleep 0.5
	...
	if [ $DOQSUB -eq 5 ];
	then
	# Run jobs on Torque and wait to finish
	echo
	echo "--------------------------------------------------------------------------------------"
	echo " Starting ANTS rigid registration on Torque cluster. Submitted $count jobs "
	echo "--------------------------------------------------------------------------------------"
        # now wait for the jobs to finish. Rigid registration is quick, so poll queue every 60 seconds
	${ANTSPATH}waitForTorqueJobs.pl 1 60 $jobIDs
...
# fi
  ...
  for IMG in $IMAGESETVARIABLE
    ...
    # 6 submit to SGE (DOQSUB=1), PBS (DOQSUB=4), Torque (DOQSUB=5), PEXEC (DOQSUB=2), XGrid (DOQSUB=3) or else run locally (DOQSUB=0)
    if [ $DOQSUB -eq 1 ]; then
      ...
      qscript="job_${count}_${i}.sh"
	     ...
	     echo "$exe" >> $qscript
      echo "cp -R /jobtmp/pbstmp.\$PBS_JOBID/* ${currentdir}" >> $qscript;
	     id=`qsub -N antsdef${i} -v ITK_GLOBAL_DEFAULT_NUMBER_OF_THREADS=1,LD_LIBRARY_PATH=$LD_LIBRARY_PATH,ANTSPATH=$ANTSPATH -l mem=4gb -l walltime=4:00:00 $QSUBOPTS $qscript | awk '{print $1}'`
	     jobIDs="$jobIDs $id"
     	sleep 0.5
	elif [ $DOQSUB -eq 4 ]; then
      qscript="job_${count}_${i}.sh"
...
#		echo " submitting job number $count " # for debugging only
  ...
  if [ $DOQSUB -eq 1 ];
      ...
      echo
      echo "--------------------------------------------------------------------------------------"
      echo " Starting ANTS registration on Torque cluster. Iteration: $itdisplay of $ITERATIONLIMIT"
      echo "--------------------------------------------------------------------------------------"

      # now wait for the stuff to finish - this will take a while so poll queue every 10 mins
      ${ANTSPATH}waitForTorqueJobs.pl 1 600 $jobIDs
