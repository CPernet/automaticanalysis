...
function fsl_diag(aap,subj)
fP = aas_getimages_bystream(aap,subj,'meanepi');
subj_dir=aas_getsubjpath(aap,subj);
structdir=fullfile(subj_dir,aap.directory_conventions.structdirname);
sP = dir( fullfile(structdir,['s' aap.acq_details.subjects(subj).structuralfn '*.nii']));
...
aas_runfslcommand(aap,sprintf('slices %s %s -s 3 -a %s.gif',rfP,rsP,iP));
...
aas_runfslcommand(aap,sprintf('slices %s %s -s 3 -a %s.gif',rsP,rfP,iP));
[img,map] = imread([iP '.gif']); s3 = size(img,1)/3;
img = horzcat(img(1:s3,:,:),img(s3+1:2*s3,:,:),img(s3*2+1:end,:,:));
imwrite(img,[iP '.jpg']); delete([iP '.gif']);
% Clean
