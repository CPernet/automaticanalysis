    ...
    if isempty(factorNum{s})
        ...
        if s > 1
            ...
            if factorNum{s} ~= factorNum{s-1} && (factorNum{s-1}~=0 && factorNum{s}~=0)
                if aap.tasklist.currenttask.settings.mergeSessions == 2
                    aas_log(aap,false,sprintf('Unequal number of factors across the two sessions', factorNum{s-1}, factorNum{s}))
                else
...
if aap.tasklist.currenttask.settings.mergeSessions == 1
    factors = [factorNum{:}];
    factors(factors == 0) = [];
    aap.tasklist.currenttask.settings.conditions = factors(1);
elseif aap.tasklist.currenttask.settings.mergeSessions == 2
    aap.tasklist.currenttask.settings.conditions = sum(cell2mat(factorNum));
else
...
tempCond = cell(1,length(conditions));

% Get rid of repeated condition names...
for s = 1:length(conditions)
    tempCond{s} = conditions{s}(~nuisanceNum{s});
    % Get rid of repeated names
    for t1 = length(tempCond{s}):-1:1
        for t2 = t1-1:-1:1
            if strcmp(tempCond{s}{t1}, tempCond{s}{t2})
                tempCond{s}(t1) = [];
                break
            end
        end
...
if aap.tasklist.currenttask.settings.mergeSessions < 2
    for s = 1:length(conditions)
        if ~isempty(tempCond{s})
            aap.tasklist.currenttask.settings.conditionNames = tempCond{s};
            break
        end
    end
else
...
prevSess = 0;
blockMergeInd = 0; % Used only if we want to merge sessions...
condMergeInd = 0; % Used only if we want to merge conditions...
for s = 1:length(SPM.Sess)
    ...
    for b=1:blockNum{s}
        blockMergeInd = blockMergeInd + 1;
        for c=1:factorNum{s}
            condMergeInd = condMergeInd + 1;
            % Do we want betas or T-values?
            if strcmp(dataType, 'betas')
                condStr =  [tempCond{s}{c} '_sub' num2str(b)];
            ...
            if aap.tasklist.currenttask.settings.mergeSessions == 2
                data{condMergeInd,blockMergeInd,1} = Y;
            elseif aap.tasklist.currenttask.settings.mergeSessions == 1
                data{c,blockMergeInd,1} = Y;
            else
                data{c,b,s} = Y;
            end
        end
